...
import { CampaignWithPatient, Donation } from "@/types/database";
...
export default function DonorDashboard() {
  ...
  const [myDonations, setMyDonations] = useState<DonationWithCampaign[]>([]);
  ...
  useEffect(() => {
    ...
    setDataLoading(true);
    Promise.all([
      getCampaigns(filter === "all" ? {} : { status: filter === "active" ? "active" : "completed" }),
      supabase
        .from("donations")
        ...
        .limit(10),
    ]).then(([camps, { data: donations }]) => {
      setCampaigns(camps);
      setMyDonations((donations ?? []) as DonationWithCampaign[]);
  ...
  return (
    ...
    >
      ...
      {/* Impact card */}
      <div
        className="bg-primary rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-6"
        style={{ boxShadow: "0 8px 32px rgba(26,107,90,0.2)" }}
      >
        <div className="flex-1">
          ...
          <p
            ...
            style={{ fontFamily: "Fraunces, serif" }}
          >
            {authLoading || dataLoading ? "—" : formatBRL(totalDonated)}
          </p>
        ...
        <div className="flex gap-6">
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-xl font-bold text-white" style={{ fontFamily: "Fraunces, serif" }}>
              {dataLoading ? "—" : myDonations.length}
            </p>
      ...
      <section aria-labelledby="donations-heading">
        ...
        {dataLoading ? (
          <div className="bg-white rounded-2xl p-5 flex flex-col gap-4" style={{ boxShadow: "0 4px 24px rgba(26,107,90,0.06)" }}>
            {[1, 2, 3].map((i) => (
          ...
          >
            ...
            <Link
              href="/campaigns"
              className="text-xs font-semibold text-accent hover:text-[#E8704F] transition-colors"
            >
              Explorar campanhas →
            </Link>
          </div>
          ...
          <div
            className="bg-white rounded-2xl divide-y divide-[#D6E8E3]"
            style={{ boxShadow: "0 4px 24px rgba(26,107,90,0.06)" }}
          >
